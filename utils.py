...
from data import policy2transform, preprocess, SubgraphData, TUDataset, PTCDataset, Sampler, policy2transform_imdb
...
def get_data(args, fold_idx):
    ...
    if 'ogb' in args.dataset:
        ...
        dataset = TUDataset(root="dataset/" + args.policy,
                            name=args.dataset,
                            pre_transform=policy2transform_imdb(policy=args.policy, num_hops=args.num_hops),
                            )
        if args.fraction != 1.:
            dataset = preprocess(dataset, transform)
...
def get_model(args, in_dim, out_dim, device, sample=None):
    encoder = lambda x: x
    if 'ogb' in args.dataset:
    ...
    if args.model == 'deepsets':
        ...
        model = DSSnetwork(num_layers=args.num_layer, in_dim=in_dim, emb_dim=args.emb_dim, num_tasks=out_dim,
                           feature_encoder=encoder, GNNConv=GNNConv, sample=sample).to(device)
